...
const UserAvatar = ({ user }: { user: Iuser }) => {
  return (
    <div
      ...
      style={{ backgroundColor: generateAvatarColor(user?.username as string) }}
    >
  ...
  return (
    <>
      {messages.map((msg, index) => (
        ...
        >
          {/* sender avatar */}
          {msg.sender !== logedinUser?._id && <UserAvatar user={selectedUserForChat as Iuser} />}
          <div
            className={`max-w-[75%] md:max-w-[60%] p-4 rounded-4xl shadow-sm relative group transition-all duration-200 ${
          ...
          {/* receiver avatar */}
          {msg.sender == logedinUser?._id && <UserAvatar user={logedinUser as Iuser} />}
        </div>
      ))}
